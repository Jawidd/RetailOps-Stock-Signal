...
print("\n1.=== Overall unit Sales Summary ===")
summary = db.execute("""
    select 
        count(distinct sales_date) as total_days,
        count(distinct store_nbr) as total_stores,
        sum(total_unit_sales) as total_unit_sales_amount,
        avg(total_unit_sales) as avg_dauily_units_sales
...
# top stores
print("\n2=== Top 10 Stores by Total units Sales ===")
top_stores = db.execute("""
    ...
    order by total_unit_sales_amount desc
    limit 10
""").fetchdf()
print(top_stores)
...
holiday_summary = db.execute("""
    select
        is_holiday,
        avg(total_unit_sales) as avg_units_sales
    from fact.fct_store_daily_sales
    group by is_holiday
""").fetchdf()
...
# sales by store type
print("\n6=== Unit Sales by Store Type ===")
store_type_summary = db.execute("""
    select
        store_type,
        sum(total_unit_sales) as total_unit_sales,
        avg(total_unit_sales) as avg_units_sales
    from fact.fct_store_daily_sales
    group by store_type
